...
export function RecurringRulesSection({ entities, create, update }: SectionProps) {
  ...
  const defaultCategory = active<SettingsData>(entities, "settings")[0]?.payload.defaultCalendarCategoryId || categories.find(item => !item.payload.archived)?.id || "";
  ...
  const [flexibility, setFlexibility] = useState<"fixed" | "flexible">("fixed"), [enabled, setEnabled] = useState(true);
  ...
  const begin = (item: CoreEntity<RecurringActivityRuleData> | null = null) => {
    ...
    setStartTime(item?.payload.startTime || "09:00"); setDuration(String(item?.payload.durationMinutes || 60)); setCategory(item?.payload.calendarCategoryId || defaultCategory); setDirection(item?.payload.directionId || "");
    setFlexibility(item?.payload.flexibility || "fixed"); setEnabled(item?.payload.active ?? true); action.clearError();
  };
  const save = async () => {
    if (!title.trim() || !category || (kind === "weekly" && !days.length)) return;
    ...
    const payload: RecurringActivityRuleData = {
      ...
      calendarCategoryId: category, directionId: direction || null, planType: editing?.payload.planType || "appointment", flexibility,
    };
  ...
  return <div className="recurring-rule-list">
    ...
    {editing !== undefined && <SectionDialog title={editing ? "繰り返し予定を編集" : "繰り返し予定を追加"} close={() => setEditing(undefined)} busy={action.busy}><form onSubmit={event => { event.preventDefault(); void save(); }}><fieldset disabled={action.busy}>
      ...
      <label>カレンダー（必須）<select required value={category} onChange={event => setCategory(event.target.value)}>{categories.filter(item => !item.payload.archived || item.id === category).map(item => <option key={item.id} value={item.id}>{item.payload.name}</option>)}</select></label>
      ...
      <label>動かせる？<select value={flexibility} onChange={event => setFlexibility(event.target.value as typeof flexibility)}><option value="fixed">固定</option><option value="flexible">柔軟</option></select></label>
      ...
      <ActionFeedback {...action}/><button className="save" type="submit" disabled={!title.trim() || !category || (kind === "weekly" && !days.length)}>保存する</button>
