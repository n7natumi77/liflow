...
export function DiaryNavigation({ tab, setTab, checks, userName, syncState, onSignOut }: {
  ...
  return <aside className="diary-navigation">
    ...
    <nav aria-label="メインメニュー" className="diary-tabs">
      {primary.map(([id, label, Icon]) => <button key={id} data-tab={id} className={tab === id || (id === "plan" && tab === "today") ? "active" : ""}
        aria-current={tab === id || (id === "plan" && tab === "today") ? "page" : undefined} onClick={() => setTab(id)}>
        <Icon size={19} /><span>{label}</span>{id === "inbox" && checks > 0 && <em>{checks > 99 ? "99+" : checks}</em>}
      </button>)}
    </nav>
    <details className="diary-menu">
      ...
      <div className="diary-menu-panel">
        ...
        {secondary.map(([id, label, Icon]) => <button key={id} data-tab={id} aria-current={tab === id ? "page" : undefined} onClick={event => { setTab(id); event.currentTarget.closest("details")?.removeAttribute("open"); }}><Icon size={17} />{label}</button>)}
