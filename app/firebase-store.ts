...
import {shiftedPlan,type ActualData,type ConflictData,type CoreEntity,type EntityType,type PlanData} from '../domain/core';
import {CURRENT_SCHEMA_VERSION,countEntities,migrateEntity,migrateSnapshot,type StoredEntity} from '../domain/schema';
...
export async function prepareUserData(uid:string){
 const entityCollection=collection(firestore,'users',uid,'entities'),snapshot=await getDocs(entityCollection),source=snapshot.docs.map(item=>entityFromDoc(item.id,item.data()));
 if(source.every(entity=>entity.schemaVersion===CURRENT_SCHEMA_VERSION))return {migrated:false,count:source.length};
 const migrated=migrateSnapshot(source),{backupId,backupRef}=await createSafetyBackup(uid,source,`schema-${CURRENT_SCHEMA_VERSION}`);
 try{
  for(let index=0;index<source.length;index++){const original=source[index],next=migrated[index];if(original.schemaVersion===CURRENT_SCHEMA_VERSION)continue;const ref=doc(entityCollection,original.id);await runTransaction(firestore,async transaction=>{const currentSnapshot=await transaction.get(ref);if(!currentSnapshot.exists())throw new Error(`migration_entity_missing:${original.id}`);const current=entityFromDoc(currentSnapshot.id,currentSnapshot.data());if(current.schemaVersion===CURRENT_SCHEMA_VERSION)return;if(current.revision!==original.revision)throw new Error(`migration_conflict:${original.id}`);transaction.set(ref,{...next,revision:current.revision+1,updatedAt:new Date().toISOString(),updatedBy:deviceId()})})}
  await setDoc(backupRef,{status:'migrated',migrationCompletedAt:new Date().toISOString()},{merge:true});return {migrated:true,count:migrated.length,backupId};
...
export async function postponePlan(uid:string,plan:CoreEntity<PlanData>,days=1){
 ...
 return runTransaction(firestore,async transaction=>{
  ...
  const nextPlan:CoreEntity<PlanData>={id:nextRef.id,type:'plan',payload:shiftedPlan(remote.payload,days),schemaVersion:CURRENT_SCHEMA_VERSION,revision:1,createdAt:now,updatedAt:now,updatedBy:by,deletedAt:null};
  const resolved:CoreEntity<PlanData>={...remote,payload:{...remote.payload,resolution:'postponed'},revision:remote.revision+1,updatedAt:now,updatedBy:by};
...
export async function recordPlanAsActual(uid:string,plan:CoreEntity<PlanData>){
 ...
 return runTransaction(firestore,async transaction=>{
  const snapshot=await transaction.get(sourceRef),remote=snapshot.exists()?migrateEntity(entityFromDoc(snapshot.id,snapshot.data())) as CoreEntity<PlanData>:plan;
  if(remote.revision!==plan.revision)throw new Error('revision_conflict');
  if(remote.deletedAt||remote.payload.resolution||remote.payload.actualId)throw new Error('already_reconciled');
  const payload:ActualData={title:remote.payload.title,taskId:remote.payload.taskId||null,planId:remote.id,projectId:remote.payload.projectId||null,calendarCategoryId:remote.payload.calendarCategoryId||null,startAt:remote.payload.startAt,endAt:remote.payload.endAt,type:remote.payload.type,note:''};
  ...
  const linkedPlan:CoreEntity<PlanData>={...remote,payload:{...remote.payload,actualId:actual.id},revision:remote.revision+1,updatedAt:now,updatedBy:by};
  transaction.set(actualRef,actual);transaction.set(sourceRef,linkedPlan);return {actual,linkedPlan};
 });
}
...
export async function recordActualForPlan(uid:string,plan:CoreEntity<PlanData>,payload:ActualData){
 ...
 return runTransaction(firestore,async transaction=>{
  ...
  if(remote.revision!==plan.revision)throw new Error('revision_conflict');
  if(remote.deletedAt||remote.payload.resolution||remote.payload.actualId)throw new Error('already_reconciled');
  const actual:CoreEntity<ActualData>={id:actualRef.id,type:'actual',payload:{...payload,planId:remote.id},schemaVersion:CURRENT_SCHEMA_VERSION,revision:1,createdAt:now,updatedAt:now,updatedBy:by,deletedAt:null};
  const linkedPlan:CoreEntity<PlanData>={...remote,payload:{...remote.payload,actualId:actual.id},revision:remote.revision+1,updatedAt:now,updatedBy:by};
  transaction.set(actualRef,actual);transaction.set(sourceRef,linkedPlan);return {actual,linkedPlan};
 });
}
...
export async function deleteActualAndUnlinkPlan(uid:string,actual:CoreEntity<ActualData>,plan:CoreEntity<PlanData>){
 ...
 return runTransaction(firestore,async transaction=>{
  ...
  if(remoteActual.revision!==actual.revision||remotePlan.revision!==plan.revision)throw new Error('revision_conflict');
  ...
  const unlinkedPlan:CoreEntity<PlanData>={...remotePlan,payload:{...remotePlan.payload,actualId:remotePlan.payload.actualId===actual.id?null:remotePlan.payload.actualId},revision:remotePlan.revision+1,updatedAt:now,updatedBy:by};
  transaction.set(actualRef,deletedActual);transaction.set(planRef,unlinkedPlan);return {deletedActual,unlinkedPlan};
 });
}
