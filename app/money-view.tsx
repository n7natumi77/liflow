...
import { active, type ActualData, type CoreEntity, type ProjectData, type TransactionData } from "../domain/core";
...
export function MoneyView({ entities, create, update }: SectionProps) {
  const transactions = active<TransactionData>(entities, "transaction"), projects = active<ProjectData>(entities, "project"), actuals = active<ActualData>(entities, "actual");
  ...
  const [project, setProject] = useState(""), [actual, setActual] = useState(""), [note, setNote] = useState("");
  ...
  const begin = (item: CoreEntity<TransactionData> | null = null) => {
    ...
    setExpected(item?.payload.expectedAt ? dateKey(new Date(item.payload.expectedAt)) : ""); setProject(item?.payload.projectId || ""); setActual(item?.payload.actualId || ""); setNote(item?.payload.note || ""); action.clearError();
  };
  const save = async () => {
    ...
    const payload = { ...editing?.payload, title: title.trim(), amount: number, direction, status, category: category.trim() || "その他", occurredAt: dateValue(occurred, editing?.payload.occurredAt), expectedAt: expected ? dateValue(expected, editing?.payload.expectedAt) : null, projectId: project || null, actualId: actual || null, planId: editing?.payload.planId || null, taskId: editing?.payload.taskId || null, note };
  ...
  return <section className="panel notebook money-notebook">
    ...
    <div className="money-ledger">{days.length ? days.map(day => <section className="money-day" key={day}><h3>{new Date(day + "T12:00:00").toLocaleDateString("ja-JP", { month: "long", day: "numeric", weekday: "short" })}</h3>
      {list.filter(t => dateKey(new Date(t.payload.occurredAt)) === day).map(t => <button className="transaction-entry" key={t.id} data-transaction-id={t.id} onClick={() => begin(t)}>
        ...
        <span className="transaction-copy"><b>{t.payload.title}</b><span className="entry-meta"><span>{t.payload.category}</span><span>{t.payload.status === "expected" ? "予定" : "確認済み"}</span>{t.payload.projectId && <span>{projects.find(p => p.id === t.payload.projectId)?.payload.name || "プロジェクトあり"}</span>}{(t.payload.planId || t.payload.actualId || t.payload.taskId) && <span><Link2 size={12}/>関連する記録あり</span>}</span>{t.payload.status === "expected" && t.payload.expectedAt && <small>入出金予定 {new Date(t.payload.expectedAt).toLocaleDateString("ja-JP")}</small>}</span>
    ...
    {editing !== undefined && <SectionDialog title={editing ? "収支を編集" : "収支を記録"} close={() => setEditing(undefined)} busy={action.busy}><form onSubmit={e => { e.preventDefault(); void save(); }}><fieldset disabled={action.busy}>
      <label>内容<input autoFocus required value={title} onChange={e => setTitle(e.target.value)}/></label><div className="form-pair">
        ...
        <label>プロジェクト<select aria-label="プロジェクト" value={project} onChange={e => setProject(e.target.value)}><option value="">なし</option>{projects.map(p => <option key={p.id} value={p.id}>{p.payload.name}</option>)}</select></label>
